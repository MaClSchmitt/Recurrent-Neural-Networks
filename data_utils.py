...
def generate_music(inference_model, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, indices_tones = indices_tones, T_y = 10, max_tries = 1000, diversity = 0.5):
    ...
    # Initializes chord variables
    curr_offset = 0.0                                     # variable used to write sounds to the Stream
    num_chords = int(len(chords) / 3)                     # number of different set of chords

    print("Predicting new values for different set of chords.")
    # Loops over all 18 set of chords. At each iteration generates a sequence of tones
    # and uses the current chords to convert it into actual sounds
    for i in range(1, num_chords):

        # Retrieves current chord from stream
        curr_chords = stream.Voice()

        # Loops over the chords of the current set of chords
        for j in chords[i]:
            # Adds chord to the current chords with the adequate offset
            curr_chords.insert((j.offset % 4), j)

        # Generates a sequence of tones using the model
        ...
        # Uses predicted tones and current chords to generate sounds
        ...
        # Prints number of tones/notes in sounds
    ...
    # Initializes tempo of the output stream with 130 bit per minute
    out_stream.insert(0.0, tempo.MetronomeMark(number=130))

    # Saves audio stream to fine
    ...
    # Plays the final stream through output
